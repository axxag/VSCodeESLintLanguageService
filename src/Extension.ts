import {
  ITSConfiguration
} from 'typescript-eslint-plugin/lib/Settings/ITSConfiguration'
import {
  DocumentFilter,
  ExtensionContext,
  extensions,
...
export class Extension {
  ...
  public async Activate(context: ExtensionContext) {
    const extension = extensions.getExtension(Extension.typeScriptExtensionId);
    if (!extension) {
      return;
    }
    ...
    workspace.onDidChangeConfiguration(
      (e) => {
        if (e.affectsConfiguration(Extension.configSection)) {
  ...
  protected UpdateConfig(api: IApiV0) {
    ...
    if (typeof config === "object") {
      ...
      let result: {
        -readonly [K in keyof ITSConfiguration]: ITSConfiguration[K];
      } = {};
